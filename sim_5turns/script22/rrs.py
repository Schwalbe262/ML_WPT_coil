...
def run_simul(version_idx_str):
    ...
    for i in np.arange(4) :
        ...
        np.save(f'..\\data\\script22\\field\\data_field_{4*version_idx_str+i}.npy', raw_data.astype(np.float) )
        ...
        np.save(f'..\\data\\script22\\input\\data_input_{4*version_idx_str+i}.npy', input.astype(np.float) )
