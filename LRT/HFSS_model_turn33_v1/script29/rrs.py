...
def run_simul(version_idx_str):
    ...
    if width_range[1] - 5*coil_width1 - 4*turn_width_gap1 > 2*coil_width1 :
    ...
    data1 = np.loadtxt(f'Z:\\Autosimul_data\\LRT\HFSS_model_turn33_v1\\{COMPUTER_NAME}\\script29\\result_data.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp))
    np.savetxt(f'Z:\\Autosimul_data\\LRT\\HFSS_model_turn33_v1\\{COMPUTER_NAME}\\script29\\result_data.csv',new_data1,delimiter=",")
