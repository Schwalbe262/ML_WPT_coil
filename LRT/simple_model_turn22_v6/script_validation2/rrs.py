...
def run_simul(version_idx_str):
    ...
    data1 = np.loadtxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\coupling.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\coupling.csv',new_data2,delimiter=",")

    data3 = np.loadtxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\loss.csv', delimiter=",")
    new_data3 = np.vstack((data3, temp3))
    np.savetxt(f'Z:\Autosimul_data\LRT\simple_model_turn22_v6\{COMPUTER_NAME}\script_validation2\loss.csv',new_data3,delimiter=",")
...
for i in range(0, 10000): 
    ...
    try :
        try:
            os.remove(f'.\ML_aedt\ML_validation2.aedt.lock')
        except:
            time.sleep(1)
        if os.path.isfile(f'.\ML_aedt\ML_validation2.aedt') :
            os.remove(f'.\ML_aedt\ML_validation2.aedt')
        time.sleep(1)	

        shutil.copy(f'.\ML_aedt\ML_ref.aedt',f'.\ML_aedt\ML_validation2.aedt')
        time.sleep(1)
        ...
        shutil.rmtree(f'.\ML_aedt\ML_validation2.aedtresults')
        try:
            os.remove(f'.\ML_aedt\ML_validation2.aedt.lock')
        except:
            time.sleep(1)
